...
router.post('/', [auth, admin], errorHandler(async (req, res) => {
    ...
    const { _id, name, description, basePrice, discountPrice, discountPercentage, insert } = product;
    //res.send({ _id, name, description, basePrice, discountPrice, discountPercentage, insert });
    res.send(_.pick(product, ['_id', 'name', 'description', 'basePrice', 'discountPrice', 'discountPercentage', 'insert', 'category']));
}));
...
router.delete('/:id', [auth, admin, idValidator], errorHandler(async (req, res) => {
    ...
    if (!product) return res.status(404).send('Catgory not found');
    res.send(product);
}));
